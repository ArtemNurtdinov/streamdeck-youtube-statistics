...
@action({ UUID: ACTION_UUIDS.lastVideoComments })
export class LastVideoCommentsAction extends BaseStatAction<ChannelSettings> {
    ...
    protected override async updateTitle(action: KeyAction<ChannelSettings>, settings: ChannelSettings): Promise<void> {
        ...
        await this.setActionTitle(action, formatNumber(latestVideo.commentCount));
    }
