...
@action({ UUID: ACTION_UUIDS.lastVideoLikes })
export class LastVideoLikesAction extends BaseStatAction<ChannelSettings> {
    ...
    protected override async updateTitle(action: KeyAction<ChannelSettings>, settings: ChannelSettings): Promise<void> {
        ...
        await this.setActionTitle(action, formatNumber(latestVideo.likeCount));
    }
